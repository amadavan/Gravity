...
int main(int argc, char *argv[]) {
    ...
    param<> voll("VoLL");
    voll.in(nodes);
    voll = 10 * max_cost; // TODO: set VoLL
    ...
    double failure_probability = 0.2 / nb_lines;
...
//  RSCED.add(Ref_Bus_recourse_c == 0);
    ...
    obj += 9999 * sum(slack_thermal_c, arcs_c) + 9999 * sum(slack_thermal_recourse_c, arcs_c)
           + 9999 * sum(slack_pad_c, node_pairs_c) + 9999 * sum(slack_pad_recourse_c, node_pairs_c)
           + 9999 * sum(slack_kcl_p_c, nodes_c) + 9999 * sum(slack_kcl_n_c, nodes_c);
